...
//    }
    ...
    func changeTextColor(text: String) -> NSMutableAttributedString {
        ...
        if inputLimit < 0 {
            attributedString.addAttribute(.foregroundColor, value: R.color.tfRed()!, range: rangeOfExtraText)
            attributedString.addAttribute(.foregroundColor, value: R.color.tfTextColor()!, range: NSRange(location: 0, length: 10))
        }
